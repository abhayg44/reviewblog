...
const updatePlace = async (req, res, next) => {
  const errors = validationResult(req);
  if (!errors.isEmpty()) {
    return next(new HttpError("Invalid inputs provided, check your data", 422));
  }
...
const deletePlace = async (req, res, next) => {
  ...
  if (!place) {
    const error = new HttpError("Place not found with the provided ID", 404);
    return next(error);
  }
  ...
  try {
    const sess = await mongoose.startSession();
    sess.startTransaction();
    deleted_place = await place.deleteOne({ session: sess });
